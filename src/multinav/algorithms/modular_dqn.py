    ...
    def __init__(
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            with tf.variable_scope("action_value"):
                ...
                action_scores = tf_layers.fully_connected(
                    action_out,
                    num_outputs=n_states * self.n_actions,
                    activation_fn=None,
                )
                action_scores = tf.reshape(
                    action_scores, shape=(n_states, self.n_actions)
                )
                indices = tf.reshape(state, shape=(1,))
                action_scores = tf.gather_nd(
                    params=action_scores, indices=indices, batch_dims=1
                )
            ...
            if self.dueling:
                with tf.variable_scope("state_value"):
                    ...
                    state_score = tf.reshape(
                        state_score, shape=(n_states, self.n_actions)
                    )
                    indices = tf.reshape(state, shape=(1,))
                    state_score = tf.gather_nd(
                        params=state_score, indices=indices, batch_dims=1
                    )
                # Dueling part
