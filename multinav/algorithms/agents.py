...
class RllibAgentModel(AgentModel):
    ...
    def __init__(self, agent_type: str, agent_conf: Dict[str, Any]):
        ...
        trainer_class = ray.rllib.agents.registry.get_trainer_class(agent_type)
        self.trainer = trainer_class(agent_conf)

    @classmethod
    def load(cls, path: str) -> "RllibAgentModel":
        """Load model from path.

        Assuming path is run-options.yaml generated by rl-experiments.
        """
        with open(path) as f:
            run_options = yaml.safe_load(f)
        agent_type = run_options["algorithm"]["params"]["agent"]
        agent_conf = run_options["algorithm"]["params"]["config"]
        agent = RllibAgentModel(agent_type, agent_conf)
        return agent
    ...
    def predict(self, observations: State) -> Action:
        """Compute next action."""
        return self.trainer.compute_action(observations)
